    ...
    def parse_single_result(self, single_result, return_type=ReturnType.FULL, **kwargs):
        ...
        if self.type in (None, "Repositories"):
            ...
            if return_type in (ReturnType.FULL,):
                ...
                lang = stars_and_lang_div.find("span", itemprop="programmingLanguage")
        ...
        if self.type == "Users":
            ...
            if return_type in (ReturnType.FULL,):
                ...
                location_and_email = location_div.find_all("div", class_="mr-3")
                location = email = None
                for single in location_and_email:
                    if single.get("href") is None:
        ...
        if self.type == "Issues":
            ...
            if return_type in (ReturnType.FULL,):
                repository = (
                    single_result.find("div", class_="ml-1").find("a", "text-bold").text
                )
                opened_by = self.base_url + single_result.find(
                    "div", class_="mr-3"
                ).find("a").get("href")
